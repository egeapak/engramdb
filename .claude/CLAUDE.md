...
- `onnxruntime` (default) — the ONNX Runtime stack (`ort` + `fastembed` + `engram-onnx`). Turn OFF (with `--no-default-features`) on a platform with no prebuilt ORT and enable `tract` instead. When off, NLI and T5 titling compile out (ORT-only) and keyword titling remains; the reranker has a pure-Rust tract path (`jina-reranker-v1-turbo-en` or `bge-reranker-base`, fp32) that stays off by default.
- **ONNX Runtime linking strategy** — `load-dynamic` (default) / `system-onnxruntime` / `bundled-onnxruntime`. EngramDB no longer vendors a runtime; exactly one of these decides where `libonnxruntime` comes from, and `load-dynamic` lives in `default`, so choosing another requires `--no-default-features --features onnxruntime,ollama,<strategy>`. `load-dynamic` `dlopen`s the runtime at *run* time (nothing needed at build time; release archives ship the library beside the binary), `system-onnxruntime` links one found via `pkg-config` at build time (what the Homebrew formula uses), `bundled-onnxruntime` downloads and statically links one (the pre-0.9 default — **avoid**: that prebuilt mis-executes quantized models on AVX-512/AMX hosts, see `docs/contributors/embedding-model-alternatives.md` R6/R9).
