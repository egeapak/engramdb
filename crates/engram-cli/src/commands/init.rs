...
pub async fn run_init(
    ...
    if let Some(origin) = &worktree_origin {
        ...
        let migrated =
            engramdb::storage::worktree::consolidate_worktree_into_main(origin, dir).await?;
        if migrated > 0 {
            formatter.print_message(&format!(
...
async fn init_model_with_backend(
    ...
    #[cfg(feature = "ollama")] ollama_spec: engramdb::embeddings::OllamaModelSpec,
    formatter: &OutputFormatter,
) {
...
mod tests {
    use super::*;
    use crate::output::OutputFormatter;
    use engramdb::storage::{paths, project_id, InMemoryRegistry, Registry, RegistryEntry};
