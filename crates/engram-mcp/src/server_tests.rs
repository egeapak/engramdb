...
/// REPRO 1c: same ordering defect in `harvest_show`.
#[tokio::test]
async fn repro_harvest_show_gate_runs_after_session_scope_writes() {
    let _home = ScopedClaudeHome::new();
    let (dir_a, dir_b, server) = setup_cross_project().await;
    write_config_toml(
        dir_a.path(),
    ...
    .await;
    engramdb::storage::harvest_state::set_archive(
        dir_b.path(),
        "victim-session",
        engramdb::storage::transcript_archive::ArchiveRef {
            file_name: "victim-session.jsonl.zst".into(),
            bytes: 10,
            original_bytes: 100,
    ...
    .unwrap();
    let before =
        std::fs::read_to_string(dir_b.path().join(".engramdb/state/harvested_sessions.json"))
            .unwrap();
    ...
    assert_eq!(
        before, after,
        "a REFUSED harvest_show mutated the target's ledger"
    );
}
