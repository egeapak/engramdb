...
| 1 | **Reranker default → `jina-reranker-v1-turbo-en`** (from `bge-reranker-base`) — **applied** | 7.3× smaller (145 MB vs 1060 MB), 3.4× faster per pair (82 vs 278 ms), 11× faster to load. And at the shipped `weight = 0.5` it is the *only* one that helps at all: bge-reranker-base reproduces the un-reranked ranking exactly, so the old default bought nothing for ~5.6 s/query. Superseded: default model is now the uint8 `jina-turbo-q` (1.55× faster, equal quality), and reranking itself flipped `enabled = true` with `top_n = 10` — ungated reranking was never worse than the baseline on any of three labeled corpora, and every gating heuristic tried made ranking worse, not better. | One-line config default. **Zero migration** — rerank scores are not persisted. |
...
| Reranker | `jina-reranker-v1-turbo-en` fp32 (`jinaai/…`) | 145 MB | superseded by the uint8 build below, still selectable | R3 below |
| Reranker | **`jina-turbo-q`** — uint8 `jinaai/jina-reranker-v1-turbo-en` | **36.5 MB** | **on by default**, `top_n = 10` | 1.55× faster than the fp32 export at equal quality; ungated reranking never lost on any measured corpus. See the `feat/rerank-on-by-default` evidence (corpus table + `top_n` knee) alongside R3. |
...
formula (maintained in a separate tap repo) carries `depends_on "onnxruntime"`
