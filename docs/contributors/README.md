...
7. [parallelization-simd.md](./parallelization-simd.md) — the CPU-bound bulk paths: what rayon bought, why the release profile (`opt-level = "z"`) defeats auto-vectorization, and why the dot product uses explicit SIMD intrinsics rather than a portable SIMD crate (`fearless_simd`, `wide` — both measured, probe at `tools/simd-probe/`).
