...
| A memory's evidence link lives in the `.md` file, not a join table | `Memory::source_sessions`, `memory_file/v2.rs` hidden block | The schema migration rebuilds the memories table *from* those files, so a LanceDB-only relation would be destroyed by the mechanism that adds columns — silently unpinning every transcript copy. Memory files are also committed and travel with a clone; `lancedb/` does not. |
| A caller that cannot enumerate pins does not evict | `transcript_archive::prune_archives(.., pinned)`, `ops::harvest_pin::evidence_links` | An unknown pin is indistinguishable from an absent one until the transcript is gone. Storage takes the set as a parameter (it may not depend on `ops`), and the unattended SessionEnd sweep skips the pass rather than guessing. "Cannot enumerate" includes a scope path that will not *stat*: `MemoryStore::open` reports `NotInitialized` both for a git worktree with no `.engramdb/` of its own (a real answer) and for a linked sub-project that was moved, deleted, or lives on an unmounted volume (no answer at all), and `session_scope` deliberately keeps registered paths that no longer exist. The directory's own readability is what separates them. |
| State files are read with `O_NOFOLLOW`, not only written with it | `crates/engram-storage/src/state_file.rs::read_state_file` | `.engramdb/` is committed and `git` checks a symlink out verbatim, so a planted ledger path is an arbitrary-file **read** whose bytes `adopt_ledger` then copies into the root project's log and the harvest tools hand to a model. Every open is also `O_NONBLOCK` + a regular-file check, because `O_NOFOLLOW` does not stop a FIFO and a `read` on one wedges the unattended SessionEnd hook. |
| The archive size budget is measured over *unpinned* copies | `transcript_archive::prune_archives` | Counting pinned bytes toward the cap would evict every unpinned copy to make room for files the sweep may not touch. Over-budget pinned bytes are reported, never enforced. |
