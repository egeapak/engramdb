...
`OnnxProvider` (`crates/engram-models/src/embeddings/onnx.rs`) is the template.

1. Implement `EmbeddingProvider` in `crates/engram-models/src/embeddings/<name>.rs`. **`model_id()` is persisted to the manifest** — distinct quantization (fp32 vs int8) MUST produce distinct IDs.
2. Add a `ModelSpec` constant and export it from `crates/engram-models/src/embeddings/mod.rs`.
...
4. Add a test in `crates/engram-models/src/embeddings/<name>.rs::tests` and add it to the `ml-models` nextest group.
...
2. CLI surface: add a `Command::MyOp` variant in `crates/engram-cli/src/app.rs`, a `run_my_op` handler in `crates/engram-cli/src/commands/<name>.rs`, re-export from `commands/mod.rs`, dispatch in `crates/engram-cli/src/lib.rs::run`.
3. MCP surface: add a `#[tool(...)]` method on `EngramDbServer` in `crates/engram-mcp/src/server.rs` with a `MyOpRequest` input struct. Resolve the target store (handle the `project` parameter), call `ops::my_op`, serialize the result.
4. Test the op in `src/ops/<name>.rs::tests` and the CLI in `crates/engram-cli/tests/cli/<name>.rs`.
...
3. **`crates/engram-cli/src/output.rs`**: if the variant needs a unique color/glyph for pretty output, wire it in.
...
1. **`crates/engram-types/src/config.rs`**: add the field with `#[serde(default = "default_my_field")]` and a `fn default_my_field() -> T` returning the default. **Don't use `#[serde(default)]`** with the type's `Default` impl unless the type has the right default — explicit `default_*` functions document the value next to the field.
...
4. **`crates/engram-types/src/config.rs`**: add the weight to `ScoringWeights` or as a top-level field in `ScoringConfig`.
...
1. **`crates/engram-cli/src/output.rs`**: extend `OutputFormat` enum with the new variant.

2. **Implement the formatting paths** — search the file for places that match on `OutputFormat` and add the new arm.

3. **`crates/engram-cli/src/app.rs`**: extend `--format` to accept the new value (Clap's `value_parser` will pick it up automatically).

4. **Document in `docs/users/cli-reference.md`** under the global flags table.

## Add support for a new hook event

1. **`crates/engram-cli/src/app.rs`**: extend `HookCommand` with the new variant.

2. **`crates/engram-cli/src/commands/hook.rs`**: add the handler. Read event JSON from stdin (use `serde_json::Value` and `.get("...")` for forward-compat), emit `additionalContext` JSON.

3. **`crates/engram-cli/src/commands/setup.rs`**: update the writer that adds hooks to `settings.json` so the new event is registered when users run `engramdb setup`.
