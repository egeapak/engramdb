...
#[cfg(test)]
mod scratch_deadlock {
    use super::*;
    use crate::storage::InMemoryRegistry;

    /// REPRO 5: `session_scope` adopts every registry path into `root_dir`.
    /// `adopt_ledger` only short-circuits on *textual* PathBuf equality, so a
    /// registry entry that spells the root differently (trailing slash, `.`,
    /// symlink) makes it adopt a directory into itself: it takes the ledger
    /// lock as the source, then takes the SAME lock again as the destination.
    /// flock(2) locks are per open-file-description, so the second acquisition
    /// blocks on the first — the process hangs, with the ledger already
    /// renamed away.
    #[tokio::test(flavor = "multi_thread")]
    async fn repro_self_adoption_deadlocks() {
        let data = tempfile::TempDir::new().unwrap();
        std::env::set_var("ENGRAMDB_DATA_DIR", data.path());
        let project = tempfile::TempDir::new().unwrap();
        let dir = project.path().to_path_buf();
        // A second, symlinked spelling of the very same directory — exactly
        // what /tmp -> /private/tmp, a symlinked $HOME, or an `init --dir
        // <symlink>` recorded in the registry produces.
        let link = std::env::temp_dir().join(format!("engramdb-alias-{}", std::process::id()));
        ...
        let registry = InMemoryRegistry::new();
        let id = crate::storage::project_id::compute_project_id(&dir);
        // Written straight into the registry, as a hand-edited (or
        // otherwise non-canonical) registry.json holds it.
        registry
            .save(&crate::storage::registry::Registry {
                projects: vec![crate::storage::registry::RegistryEntry {
                    project_id: id.clone(),
                    project_path: alias.to_string_lossy().to_string(),
        ...
        crate::storage::harvest_state::mark_harvested(
            &dir,
            "s1",
            &[],
            crate::storage::harvest_state::HarvestDecision::Skipped,
            None,
        )
        .unwrap();

        let fut = session_scope(&dir, &registry);
        let out = tokio::time::timeout(std::time::Duration::from_secs(5), fut).await;
        assert!(
            out.is_ok(),
            "session_scope deadlocked adopting {} into {}",
            dir.display(),
            alias.display()
        );
        let _ = out.unwrap().unwrap();
    }
}
